...
public class MemberController {
     ...
     * @autor 송원선
     * 마이페이지 - 기본 인적사항
     * @respnse - MyPageInfoResponse : 이름, 부서명, 직급, 아이디, 사진, 흥미, 레이팅
     */
    ...
    public ResponseEntity<List<MyPageProceedingMatchResponse>> getMyPageProceedingMatchInfo(Principal principal, HttpServletRequest request) {
        ...
        // 2. 조회한 멤버 아이디로 진행 중인 매치 리스트 가져오기
        try {
            response = memberService.getMyPageProceedingMatch(memberId);
            if (response != null) {
                log.info("response -> " + response.toString());
            } else {
                log.warn("현재 진행 중 인 경기 기록이 존재하지 않습니다. " + memberId);
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            e.printStackTrace();
